...
export async function createBook(book: Omit<TherapyBook, 'id' | 'dateAdded'>): Promise<TherapyBook> {
  const id = nanoid()
...
export async function getAllBooks(): Promise<TherapyBook[]> {
  ...
  return books
    .filter((book): book is TherapyBook => {
      ...
      const hasRequiredFields = 'id' in book && 
                              'title' in book && 
                              'author' in book && 
                              'description' in book && 
                              'orderIndex' in book
...
export async function createChapter(chapter: Omit<TherapyChapter, 'id' | 'dateAdded'>): Promise<TherapyChapter> {
  const id = nanoid()
...
export async function getChaptersByBookId(bookId: string): Promise<TherapyChapter[]> {
  // Get all chapters
  ...
  return chapters
    .filter((chapter): chapter is TherapyChapter => 
      chapter !== null && typeof chapter === 'object' && 'bookId' in chapter && chapter.bookId === bookId)
    .sort((a, b) => a.orderIndex - b.orderIndex)
}

export async function getChapterById(chapterId: string): Promise<TherapyChapter | null> {
...
// Update functions
export async function updateChapter(id: string, chapter: Partial<TherapyChapter>): Promise<TherapyChapter | null> {
